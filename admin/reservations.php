            ...
            function getAllTodayReservations() {
                ...
                $.post("/api/getAllReservationsPerStatus.php",{
                    ...
                    pendingReservations.forEach( (reservation, i) => {
                        // console.log(reservation.booking.costTotal, reservation.booking.costDiscounted);
                        var down;
                        if (reservation.booking.isDiscounted === 'true') {
                            down = Math.round(parseInt(reservation.booking.costDiscounted)) / 2;
                        } else {
                            down = parseInt(reservation.booking.costTotal.replaceAll(',', '')) / 2;
                        }
                        ...
                        tableData[i] = {
                            ...
                            total: reservation.booking.isDiscounted === 'true'? reservation.booking.costDiscounted : reservation.booking.costTotal,
                            downPayment: new Intl.NumberFormat().format(Math.round(down)) + '.00',
